...
var imAccessKeyId string = ""

// 请填写您的AccessKeySecret。
var accessKeySecret string = ""
var imAccessKeySecret string = ""
...
var imCallbackUrl string = ""

// 一个小时
var expire_time int64 = 60 * 60
...
func Init() {
	...
	imAccessKeyId = conf.GetString("oss.im.accessKeyId")
	imAccessKeySecret = conf.GetString("oss.im.accessKeySecret")
	imHost = conf.GetString("oss.im.host")
	imCallbackUrl = conf.GetString("oss.im.callbackUrl")
}
...
//获取签名
func Get_policy_token(upload_dir string) PolicyToken {
...
func Get_im_policy_token(upload_dir string) PolicyToken {
	...
	result, _ := json.Marshal(config)
	debyte := base64.StdEncoding.EncodeToString(result)
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(imAccessKeySecret))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))
	...
	var callbackParam CallbackParam
	if callbackUrl != "" {
		callbackParam.CallbackUrl = imCallbackUrl
		callbackParam.CallbackBody = "filename=${object}&size=${size}&mimeType=${mimeType}&height=${imageInfo.height}&width=${imageInfo.width}"
		callbackParam.CallbackBodyType = "application/x-www-form-urlencoded"
		callback_str, err := json.Marshal(callbackParam)
		if err != nil {
	...
	policyToken.AccessKeyId = imAccessKeyId
	policyToken.Host = imHost
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = upload_dir
	policyToken.Policy = string(debyte)
